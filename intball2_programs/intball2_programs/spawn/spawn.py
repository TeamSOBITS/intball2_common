...
from intball2_programs.spawn.sim_models import (
...
def _sdf_model_names():
    share_dir = get_package_share_directory('intball2_programs')
    models_dir = os.path.join(share_dir, 'models')
    if not os.path.isdir(models_dir):
        return []
    return [os.path.splitext(f)[0] for f in os.listdir(models_dir) if f.endswith('.sdf')]


def available_model_names():
    """-hのhelp文字列用。LIGHT_MODELS/SIM_MODELS/models/*.sdfを都度読み直すため、
    どれかに項目が増減すれば次回の-h実行時から自動で反映される。
    """
    names = set(LIGHT_MODELS) | set(SIM_MODELS) | set(_sdf_model_names())
    return sorted(names)


def available_frame_names():
    """-hのhelp文字列用。spawn_locations.yamlを都度読み直し、照明用フレーム
    (iss_light_*)は-fの対象として案内する意味が薄いため除外する。
    """
    names = {DEFAULT_FRAME}
    share_dir = get_package_share_directory('intball2_programs')
    location_path = os.path.join(share_dir, 'locations', 'spawn_locations.yaml')
    try:
        with open(location_path, 'r', encoding='utf-8') as handle:
            data = yaml.safe_load(handle) or {}
        names |= {name for name in data if not name.startswith('iss_light')}
    except (OSError, yaml.YAMLError):
        pass
    return sorted(names)


def _wrap_choices(names, width=72, indent='  '):
...
def parse_args(argv):
    parser = argparse.ArgumentParser(
        description='Spawn an SDF model in Gazebo.',
        formatter_class=argparse.RawTextHelpFormatter,
    )
    parser.add_argument(
        '-m', dest='model', default=DEFAULT_MODEL,
        help='モデル名 (デフォルト: %(default)s)\n選択可能:\n' + _wrap_choices(available_model_names()),
    )
    parser.add_argument('-n', dest='name', default=None, help='モデルインスタンス名 (省略時は自動生成)')
    parser.add_argument(
        '-f', dest='frame', default=DEFAULT_FRAME,
        help='参照フレーム (デフォルト: %(default)s)\n選択可能:\n' + _wrap_choices(available_frame_names()),
    )
