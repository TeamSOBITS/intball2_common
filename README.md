...
```sh
ros2 run intball2_programs fan_control [引数]
```
...
ros2 run intball2_programs fan_control --fan 1 --duty 0.5 --duration 2

# ファン毎に個別指定（fan1=0.5, fan3=0.2）
ros2 run intball2_programs fan_control --set 1:0.5 3:0.2 --duration 2

# 全ファンを duty=0.3 で 1秒間 駆動
ros2 run intball2_programs fan_control --all 0.3 --duration 1

# 逆向きペアで往復（並進をおおよそ初期位置に戻す: 押す→2倍戻す→止める）
ros2 run intball2_programs fan_control --fan 1 --duty 0.3 --duration 1 && \
ros2 run intball2_programs fan_control --fan 8 --duty 0.3 --duration 2 && \
ros2 run intball2_programs fan_control --fan 1 --duty 0.3 --duration 1
```
